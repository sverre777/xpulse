...
export function StandardSessionsTab({ targetUserId }: { targetUserId?: string }) {
  ...
  if (series === null) {
    return <p style={{ fontFamily: "'Barlow Condensed', sans-serif", color: '#8A8A96', fontSize: 14 }}>Laster standardøkter…</p>
  }
  ...
  if (series.length === 0) {
    return (
      <div className="p-8 text-center" style={{ border: '1px dashed var(--line2)', borderRadius: 14 }}>
        <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, letterSpacing: '0.05em', color: '#F0F0F2', marginBottom: 8 }}>
          ⟳ Standardøkter
        </p>
        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 14.5, color: '#8A8A96', maxWidth: 520, margin: '0 auto', lineHeight: 1.6 }}>
    ...
    return (
      <div className="space-y-4">
        <button type="button" onClick={() => { setSelectedId(null); setEditing(false) }}
          ...
          style={{ fontFamily: "'Barlow Condensed', sans-serif", color: '#8A8A96', background: 'none', border: '1px solid var(--line2)', borderRadius: 999, padding: '7px 14px', cursor: 'pointer' }}>
          ← Alle serier
        </button>
          ...
          {editing ? (
            <div className="flex flex-col gap-2" style={{ maxWidth: 420 }}>
              <input value={editName} onChange={e => setEditName(e.target.value)} placeholder="Navn"
                style={{ background: 'var(--card2)', border: '1px solid var(--line)', borderRadius: 8, color: '#F0F0F2', fontFamily: "'Barlow Condensed', sans-serif", fontSize: 15, padding: '9px 11px', outline: 'none' }} />
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => { void saveEdit() }}
                  className="text-xs tracking-widest uppercase"
                  style={{ fontFamily: "'Barlow Condensed', sans-serif", color: '#0A0A0B', background: ACCENT, border: 'none', borderRadius: 999, padding: '8px 16px', cursor: 'pointer', fontWeight: 700 }}>
                  Lagre
                </button>
                <button type="button" onClick={() => setEditing(false)}
                  className="text-xs tracking-widest uppercase"
                  style={{ fontFamily: "'Barlow Condensed', sans-serif", color: '#8A8A96', background: 'none', border: '1px solid var(--line2)', borderRadius: 999, padding: '8px 16px', cursor: 'pointer' }}>
                  Avbryt
                </button>
              </div>
            </div>
            ...
            <>
              <div className="flex flex-wrap items-center gap-2">
                <h3 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 26, letterSpacing: '0.04em', color: '#F0F0F2' }}>
                  ⟳ {selected.name}
                </h3>
                {!readOnly && (
              ...
              <p className="mt-1" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 13.5, color: '#8A8A96' }}>
                {[sportLabel, selected.movement_name, selected.location,
                  `${selected.workout_count} gjennomføring${selected.workout_count !== 1 ? 'er' : ''}`]
                  .filter(Boolean).join(' · ')}
              </p>
              {selected.description && (
                <p className="mt-1" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 13.5, color: '#C0C0CC' }}>
                  {selected.description}
                </p>
              )}
            </>
        ...
        <div className="p-4" style={{ backgroundColor: 'var(--card)', border: '1px solid var(--line)', borderRadius: 14 }}>
          ...
          {selected.executions.length === 0 ? (
            <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 13.5, color: '#555560' }}>
            ...
            <div className="flex flex-col">
              {[...selected.executions].reverse().map(e => (
                <div key={e.workout_id} className="flex flex-wrap items-center gap-x-4 gap-y-1 py-2"
                  ...
                  <span style={{ color: '#F0F0F2', minWidth: 90 }}>{fmtDate(e.date)}</span>
                  <span style={{ color: '#C0C0CC', flex: 1, minWidth: 120 }}>{e.title || '—'}</span>
                  <span style={{ color: '#8A8A96' }}>{fmtDur(e.total_seconds)}</span>
                  {e.distance_meters != null && e.distance_meters > 0 && (
                    <span style={{ color: '#8A8A96' }}>{(e.distance_meters / 1000).toLocaleString('nb-NO', { maximumFractionDigits: 1 })} km</span>
                  )}
                  {e.avg_heart_rate != null && <span style={{ color: '#8A8A96' }}>ø{e.avg_heart_rate}</span>}
                </div>
              ))}
            </div>
          )}
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Søk navn, sted eller bev.form…"
          style={{ background: 'var(--card2)', border: '1px solid var(--line)', borderRadius: 8, color: '#F0F0F2', fontFamily: "'Barlow Condensed', sans-serif", fontSize: 14, padding: '9px 12px', outline: 'none', minWidth: 220 }} />
        {sportsPresent.length > 1 && (
          <select value={sportFilter} onChange={e => setSportFilter(e.target.value)}
            style={{ background: 'var(--card2)', border: '1px solid var(--line)', borderRadius: 8, color: '#F0F0F2', fontFamily: "'Barlow Condensed', sans-serif", fontSize: 14, padding: '9px 10px', outline: 'none' }}>
          ...
          {filtered.map(r => {
            ...
            return (
              <button key={r.id} type="button" onClick={() => setSelectedId(r.id)}
                ...
                <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 12.5, color: '#555560' }}>
