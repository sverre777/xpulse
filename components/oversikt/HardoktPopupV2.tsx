...
import { fraTidspunktNotater, type GrafPunkt } from '@/components/workout/Punkt'
...
export function HardoktPopupV2({ w, klokke, onClose }: { w: OversiktWorkoutCard; klokke: WorkoutKlokkesyncData | null; onClose: () => void }) {
  // Punktene til blokk-grafen: laktat og ernæring fra klokkepakken, pluss
  // tidspunktnotatene. Samme former og farger som på kurven.
  const grafPunkter: GrafPunkt[] = [
    ...(klokke?.lactate ?? []).map((l, i) => ({
      id: `pl${i}`, sek: l.t, slag: 'laktat' as const, planlagt: false,
      tittel: `${String(l.mmol).replace('.', ',')}`,
    })),
    ...(klokke?.nutrition ?? []).map((n, i) => ({
      id: `pn${i}`, sek: n.t, slag: 'ernaering' as const, planlagt: false,
      tittel: n.carbs_g != null ? `${n.carbs_g} g` : (n.type ?? 'Ernæring'),
    })),
    ...fraTidspunktNotater(klokke?.tidspunktNotater),
  ]
      ...
      <div data-hardokt-popup>
        <div data-popup-graf={harKurve ? 'kurve' : 'blokker'}>
          {harKurve && klokke && klokke.samples && klokke.sport ? (
            ...
            /* Uten pulskurve tegnes blokkene - punktene (laktat, ernæring,
               notat) skal likevel stå på tidslinja (Sverre 14. sep: «dråpen
               vises ikke i grafen»). */
            <PlanGraf blokker={blokker} tetthet="full" hoyde={160} punkter={grafPunkter} />
          )}
        </div>
                  ...
                  {laps.map((l, i) => {
                    ...
                    return (
                      <div key={l.id ?? i} style={{ display: 'contents' }} data-runde={l.index}>
                        ...
                        <span style={{ ...tall, display: 'inline-flex', alignItems: 'center', gap: 4 }}>{skyting && harSki && <Ikon navn="skyting" variant="fyll" storrelse={14} />}{l.index}</span>
            ...
            {((klokke?.lactate.length ?? 0) > 0 || (klokke?.nutrition.length ?? 0) > 0 || w.lactate_mmol != null) && (
              <PopupSeksjon tittel="Laktat & ernæring">
                <div className="flex flex-col gap-1" data-popup-laktat-ernaering>
                  {(klokke?.lactate ?? []).map((l, i) => (
                    <span key={`l${i}`} style={tekst}><b style={{ color: LAKTAT, display: 'inline-flex', alignItems: 'center', gap: 4 }}><Ikon navn="laktat" variant="fyll" storrelse={14} /> {fmtMmol(l.mmol)} mmol</b> ved {fmtKlokke(l.t)}</span>
                  ))}
                  {(klokke?.lactate.length ?? 0) === 0 && w.lactate_mmol != null && <span style={tekst}><b style={{ color: LAKTAT, display: 'inline-flex', alignItems: 'center', gap: 4 }}><Ikon navn="laktat" variant="fyll" storrelse={14} /> {fmtMmol(w.lactate_mmol)} mmol</b> (høyeste)</span>}
                  {(klokke?.nutrition ?? []).map((n, i) => (
                    <span key={`n${i}`} style={tekst}><b style={{ color: ERNAERING, display: 'inline-flex', alignItems: 'center', gap: 4 }}><Ikon navn="ernaering" variant="fyll" storrelse={14} /> {n.carbs_g != null ? `${Math.round(n.carbs_g)} g karbo` : n.type}</b> ved {fmtKlokke(n.t)}{n.carbs_g != null && n.type ? ` · ${n.type}` : ''}</span>
                  ))}
                </div>
              </PopupSeksjon>
            )}
