...
/**
 * Skytelinje. SELVSKJULENDE (notat pkt 7): uten skudd rendres ingenting,
 * heller ikke overskriften — en løper skal aldri se en tom skyteboks.
 * Treff % vises bare når treff faktisk er ført; ellers «—», aldri 0 %.
 */
export function ShotLine({ shots, label = 'Skyting' }: {
  ...
  return (
    <div className="flex items-center gap-2"
      style={{
        ...
        fontFamily: FONT, fontSize: 11.5, color: '#8B8B95',
      }}>
      <span style={{ width: 6, height: 6, borderRadius: '50%', background: COLOR_STANDING, display: 'block' }} />
      {label} <em style={{ fontStyle: 'normal', color: '#F2F2F0', fontSize: 12 }}>{shots.shots}</em> skudd
      {shots.accuracy_pct != null ? (
        <> · <em style={{ fontStyle: 'normal', color: '#F2F2F0', fontSize: 12 }}>{shots.accuracy_pct} %</em> treff</>
      ) : (
        <> · <span style={{ color: '#55555F' }}>— treff ikke ført</span></>
      )}
    </div>
  )
}
