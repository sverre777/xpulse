...
export function computeActivityTotals(
  ...
  for (const a of activities) {
    ...
    // Når HR-soner er eksplisitt satt, ikke fall tilbake til puls. Hurtighet
    // alene utløser ikke fallback-suppression fordi den ikke stemmer med puls.
    if (hasExplicitHr) continue
    void hasExplicitAny
  ...
  totals.zoneTotalSec = ALL_ZONE_NAMES.reduce((s, k) => s + totals.zoneSeconds[k], 0)
  return totals
}
...
export function mergeTotals(parts: ActivityTotals[]): ActivityTotals {
  ...
  out.zoneTotalSec = ALL_ZONE_NAMES.reduce((s, k) => s + out.zoneSeconds[k], 0)
  return out
}
