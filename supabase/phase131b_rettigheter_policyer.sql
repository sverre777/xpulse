...
/*   standard_session_series   SELECT -> can_view_analysis (leser kolonnen)   */ 
/*   workout_nutrition_entries SELECT -> can_view_dagbok (leser kolonnen)      */ 
/*   workout_weather           SELECT -> can_view_dagbok (leser kolonnen)      */ 
...
/*                             (regel 21) - droppes                           */ 
/*                                                                            */ 
/* TELLINGEN bruker pg_depend (det Postgres selv sjekker for drop), ikke      */ 
/* regex pa qual - nutrition og weather bruker et annet alias enn «r.» og     */ 
/* forsvant fra en regex-telling (malt 17. sep: pg_depend gir 29 policyer).   */ 
/*                                                                            */ 
...
select 'FOR' as steg, 
  (select count(distinct pol.oid) from pg_policy pol 
     ...
     where rc.relname = 'coach_athlete_relations' 
       and a.attname in ('can_edit_plan','can_view_dagbok','can_view_analysis','can_edit_periodization')) as policyer_pa_relasjonsflagg,   /* forventet 29 for, 0 etter blokk 4 */ 
  ...
  (select count(*) from pg_proc f join pg_namespace n on n.oid = f.pronamespace 
     where n.nspname = 'public' 
       and f.prosrc ~ 'can_(edit_plan|view_dagbok|view_analysis|edit_periodization)' 
       and f.prosrc !~ 'coach_data_permissions') as funksjoner_pa_relasjonsflagg,   /* forventet 2 for, 0 etter blokk 2 */ 
...
/* DROP-LINJA HORER TIL BLOKK 4a - ta den med nar blokka limes inn. */ 
drop policy if exists "Trener med plan-rett" on public.user_thresholds; 

do $$ 
begin 
  if exists (select 1 from pg_policies where schemaname = 'public' 
              and tablename = 'user_thresholds' and policyname = 'Trener med plan-rett') then 
    raise exception 'BLOKK 4a: «Trener med plan-rett» star fortsatt - drop-linja ble ikke kjort'; 
  end if; 
end $$; 
...
select 'BLOKK 4a' as steg, policyname, cmd from pg_policies 
 where schemaname = 'public' and tablename = 'user_thresholds' order by cmd, policyname; 
  /* forventet: Own-policyen + Trener leser + skriver/oppdaterer/sletter - ingen «plan-rett» */ 
...
/* SELECT (malt i blokk 0), de seks skitest-policyene og de tre SELECT-ene   */ 
/* som leser et se-flagg. 27 statements, en per policy, atomisk - ingen drop.*/ 
/* ═══════════════════════════════════════════════════════════════════════ */ 
...
/* De tre SELECT-policyene som leser en av kolonnene blokk 6 dropper (funnet */ 
/* 17. sep av pg_depend - Postgres hadde nektet droppen). Samme se-flagg,    */ 
/* ny kilde. USING alene: SELECT har ingen WITH CHECK.                       */ 
alter policy "Coach reads athlete standard session series" on public.standard_session_series 
  using (exists ( 
      select 1 from public.coach_athlete_relations r 
      join public.coach_data_permissions p on p.coach_athlete_relation_id = r.id 
      where r.athlete_id = standard_session_series.user_id 
        and r.coach_id = auth.uid() and r.status = 'active' 
        and (p.can_view_analysis) 
    )); 
alter policy "Coach read access" on public.workout_nutrition_entries 
  using (exists ( 
      select 1 from public.coach_athlete_relations r 
      join public.coach_data_permissions p on p.coach_athlete_relation_id = r.id 
      where r.athlete_id = workout_nutrition_entries.user_id 
        and r.coach_id = auth.uid() and r.status = 'active' 
        and (p.can_view_dagbok) 
    )); 
alter policy "Coach read access" on public.workout_weather 
  using (exists ( 
      select 1 from public.coach_athlete_relations r 
      join public.coach_data_permissions p on p.coach_athlete_relation_id = r.id 
      where r.athlete_id = workout_weather.user_id 
        and r.coach_id = auth.uid() and r.status = 'active' 
        and (p.can_view_dagbok) 
    )); 
...
select 'BLOKK 4c' as steg, 
  (select count(distinct pol.oid) from pg_policy pol 
     ...
     join pg_attribute a on a.attrelid = d.refobjid and a.attnum = d.refobjsubid 
     where rc.relname = 'coach_athlete_relations' 
       and a.attname in ('can_edit_plan','can_view_dagbok','can_view_analysis','can_edit_periodization')) as igjen_pa_relasjonsflagg; 
...
begin 
  /* pg_depend: det Postgres selv sjekker for drop, uavhengig av alias. */ 
  select (select count(distinct pol.oid) from pg_policy pol 
     join pg_depend d on d.objid = pol.oid and d.classid = 'pg_policy'::regclass 
     join pg_class rc on rc.oid = d.refobjid 
     join pg_attribute a on a.attrelid = d.refobjid and a.attnum = d.refobjsubid 
     where rc.relname = 'coach_athlete_relations' 
       and a.attname in ('can_edit_plan','can_view_dagbok','can_view_analysis','can_edit_periodization')) into n_pol; 
  if n_pol <> 0 then 
    raise exception 'ASSERTION 1 FEILET: % policy(er) avhenger fortsatt av flaggkolonnene pa coach_athlete_relations (lista under blokka viser hvilke)', n_pol; 
  end if; 

  select count(*) into n_fn from pg_proc f join pg_namespace n on n.oid = f.pronamespace 
   where n.nspname = 'public' 
     and f.prosrc ~ 'can_(edit_plan|view_dagbok|view_analysis|edit_periodization)' 
     and f.prosrc !~ 'coach_data_permissions'; 
  if n_fn <> 0 then 
    raise exception 'ASSERTION 2 FEILET: % funksjon(er) leser flaggene uten a ga via coach_data_permissions', n_fn; 
  end if; 
  ...
  raise notice 'ASSERTIONS OK: 0 policyer (pg_depend) og 0 funksjoner pa relasjonsflagg, INSERT-policy finnes, alle relasjoner har rad'; 
end $$; 

/* Hvis assertion 1 feiler: denne lister hvem (samme sporring som Cowork sendte). */ 
select c.relname as tabell, pol.polname as policy, a.attname as kolonne 
  from pg_policy pol 
  ...
  join pg_attribute a on a.attrelid = d.refobjid and a.attnum = d.refobjsubid 
 where rc.relname = 'coach_athlete_relations' 
   and a.attname in ('can_edit_plan','can_view_dagbok','can_view_analysis','can_edit_periodization') 
 order by 1, 2;   /* forventet 0 rader */ 
...
select 'ETTER' as steg, 
  ...
  (select count(distinct pol.oid) from pg_policy pol 
     ...
     join pg_attribute a on a.attrelid = d.refobjid and a.attnum = d.refobjsubid 
     where rc.relname = 'coach_athlete_relations' 
       and a.attname in ('can_edit_plan','can_view_dagbok','can_view_analysis','can_edit_periodization')) as policyer_pa_relasjonsflagg,   /* 0 - kolonnene finnes ikke lenger */ 
  (select count(*) from pg_policies where schemaname = 'public' 
     and (coalesce(qual,'') || coalesce(with_check,'')) ~ 'coach_data_permissions') as policyer_pa_rettighetstabellen,   /* 18 + 6 + 3 + 4 + 3 + 1 + fase 59-ene (2) = 37 - les tallet */ 
  (select count(*) from public.coach_athlete_relations) as relasjoner, 
  (select count(*) from public.coach_data_permissions) as rettighetsrader, 
